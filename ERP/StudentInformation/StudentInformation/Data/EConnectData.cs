...
namespace Southville.GP.Data
{
    class EConnectData
    {
        private static string sConnectionString = @"data source=sisc-crusaders;initial catalog=TWO;integrated security=SSPI;persist security info=False;packet size=4096";
        private static eConnectMethods eConnect = new eConnectMethods();
        ...
        public bool createOrUpdateCustomer(Customer c)
        {
            ...
            try
            {
                ...
                xml.Append("<CUSTNMBR>" + c.CustomerID + "</CUSTNMBR>");
                xml.Append("<CUSTNAME>" + c.CustomerName + "</CUSTNAME>");
                xml.Append("<CUSTCLAS>" + c.CustomerClass + "</CUSTCLAS>");
                xml.Append("<INACTIVE>" + c.Inactive.GetHashCode() + "</INACTIVE>");

                if (c.ShortName != null) xml.Append("<SHRTNAME>" + c.ShortName + "</SHRTNAME>");
                if (c.StatementName != null) xml.Append("<STMTNAME>" + c.StatementName + "</STMTNAME>");
                if (c.ShipTo != null) xml.Append("<PRSTADCD>" + c.ShipTo + "</PRSTADCD>");
                if (c.StatementTo != null) xml.Append("<STADDRCD>" + c.StatementTo + "</<STADDRCD>");
                if (c.BillTo != null) xml.Append("<PRBTADCD>" + c.BillTo + "</PRBTADCD>");
                if (c.Type != null) xml.Append("<USERDEF1>" + c.Type + "</USERDEF1>");
                if (c.StudentStatus != null) xml.Append("<USERDEF2>" + c.StudentStatus + "</USERDEF2>");
 ...
 */
                if (c.CustomerAddress.AddressString != null)
                {
                    ...
                    {
                        xml.Append("<ADDRESS1>" + c.CustomerAddress.AddressString + "</ADDRESS1>");
                    }
                    else
                    {
                        xml.Append("<ADDRESS1>" + c.CustomerAddress.AddressString.Substring(0, 50) + "</ADDRESS1>");
                        xml.Append("<ADDRESS2>" + c.CustomerAddress.AddressString.Substring(50,c.CustomerAddress.AddressString.Length-50) + "</ADDRESS2>");
                    }
                }
                if (c.CustomerAddress.AddressCode != null) xml.Append("<ADRSCODE>" + c.CustomerAddress.AddressCode + "</ADRSCODE>");
                if (c.CustomerAddress.AddressContact != null) xml.Append("<CNTCPRSN>" + c.CustomerAddress.AddressContact + "</CNTCPRSN>");
                if (c.CustomerAddress.City != null) xml.Append("<CITY>" + c.CustomerAddress.City + "</CITY>");
                if (c.CustomerAddress.State != null) xml.Append("<STATE>" + c.CustomerAddress.State + "</STATE>");
                if (c.CustomerAddress.Country != null) xml.Append("<COUNTRY>" + c.CustomerAddress.Country + "</COUNTRY>");
                if (c.CustomerAddress.Zipcode != null) xml.Append("<ZIPCODE>" + c.CustomerAddress.Zipcode + "</ZIPCODE>");
                if (c.CustomerAddress.PhoneNumber1 != null) xml.Append("<PHNUMBR1>" + c.CustomerAddress.PhoneNumber1 + "</PHNUMBR1>");
                if (c.CustomerAddress.PhoneNumber2 != null) xml.Append("<PHNUMBR2>" + c.CustomerAddress.PhoneNumber2 + "</PHNUMBR2>");
                if (c.CustomerAddress.FaxNumber != null) xml.Append("<FAX>" + c.CustomerAddress.FaxNumber + "</FAX>");
